...
public class MainActivity extends Activity {
    ...
    /**
     * NTag424DNA card object.
     */
    private INTAG424DNA nTAG424DNA;
...
//            showDisclaimer();
    ...
    private void cardLogic(final Intent intent) {
        ...
        try {
            ...
            // added by AndroidCrypto
            Log.i(TAG, "### CardType: " + type.getTagName());
        } catch (NxpNfcLibException ex) {
            Toast.makeText(this, ex.getMessage(), Toast.LENGTH_SHORT).show();
        }
        ...
        switch (type) {
            ...
            // added by AndroidCrypto
            case NTAG424DNA:
                mCardType = CardType.NTAG424DNA;
                nTAG424DNA = DESFireFactory.getInstance().getNTAG424DNA(libInstance.getCustomModules());
                showMessage("Card Detected: NTAG424DNA", 't');
                try {
                    if (!nTAG424DNA.getReader().isConnected())
...
//            plusEV1SL1.sectorSwitchToSL3(objKEY_AES128,0x4005, switchMap);
    ...
    /**
     * added by AndroidCrypto as not available within example
     */

    private void nTAG424DNACardLogic() {
        int timeOut = 2000;
        ...
        tv.setText(" ");
        showImageSnap(R.drawable.desfire_ev2);
        showMessage("Card Detected : " + nTAG424DNA.getType().getTagName(), 'n');

        try {
            nTAG424DNA.getReader().setTimeout(timeOut);
            showMessage("Version of the Card : " + Utilities.dumpBytes(nTAG424DNA.getVersion()),'d');
            ...
            byte[] pCDcap2Lrp = new byte[6];
            pCDcap2Lrp[0] = (byte) 0x02; // setting the LRP switch
            byte[] appDFName = { (byte) 0xD2, (byte) 0x76, 0x00, 0x00, (byte) 0x85, 0x01, 0x01 };
            nTAG424DNA.isoSelectApplicationByDFName(appDFName);

            // get key version of key 0
            byte kVersion = nTAG424DNA.getKeyVersion(0);
            showMessage("keyVersion of key 0 : " + Utilities.dumpHex(kVersion, " "),'d');

            // status: WORKING on both AES and LRP secure messaging

            // this is regular AES secure messaging mode
            //nTAG424DNA.authenticateEV2First(0, objKEY_AES128ZERO, pCDcap2);
            //showMessage("nTAG424DNA.authenticateEV2First done",'d');
            //showMessage("new pCDcap2 : " + Utilities.dumpBytes(pCDcap2),'d');

            // this is LRP secure messaging mode
            nTAG424DNA.authenticateLRPFirst(0, AES_KEY_ZERO, pCDcap2Lrp);
            showMessage("nTAG424DNA.authenticateLRPEFirst done",'d');
            showMessage("new pCDcap2 : " + Utilities.dumpBytes(pCDcap2Lrp),'d');
